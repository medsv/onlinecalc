...
with st.expander("Схема"):
    st.image("img/Кавитационный_запас_Схема.png", width = 200)
    st.markdown("P - абсолютное давление над поверхностью воды, Па;\n\nt - температура воды, °С;\n\n" \
    "H - подпор, разница отметок поверхности воды и оси вала насоса (может иметь отрицательное значение), м;\n\n" \
        "dH - потери давления (напора) в подводящем трубопроводе, м.")

p: float = st.number_input("Давление P, Па", value=101325.0, step=1., min_value=100000., max_value=20e6, key ="p", width = 200)
t: float = st.number_input("Температура t, °С", value=20., step=1., min_value=-100., max_value=1000., key ="t", width = 200)
